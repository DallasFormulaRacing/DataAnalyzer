...
import dataanalyzer.lambdamap.LambdaMapSettings;
    ...
    private void updateTables() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            try {
                ...
                list.addLast(rpmObj);
                rpm = ((SimpleLogObject) rpmObj).getValue();

                LogObject tpsObj = list2.pop();
                list2.addLast(tpsObj);
                tps = ((SimpleLogObject) tpsObj).getValue();

                LogObject lambdaObj = list3.pop();
                list3.addLast(lambdaObj);
                lambda = ((SimpleLogObject) lambdaObj).getValue();

                LogObject injectorObj = list4.pop();
                list4.addLast(injectorObj);
                injectorTime = ((SimpleLogObject) injectorObj).getValue();
            } catch (Exception e) {
