    ...
    public void addLap(Lap l) {
        ...
        for(DatasetSelection ds : datasetSelections) {
            if(ds.selectedTags.isEmpty())
                continue;
            Lap myLap = l.clone();
            myLap.setStart(roundToNearestLogObject(myLap.getStart(), ds.dataset.getDataMap().getList(ds.selectedTags.get(0))));
            myLap.setStop(roundToNearestLogObject(myLap.getStop(), ds.dataset.getDataMap().getList(ds.selectedTags.get(0))));
            //check that the lap is within the limits.
            if(myLap.getStart() < ds.dataset.getDataTimeLength() && myLap.getStop() < ds.dataset.getDataTimeLength()) {
                //add the created lap
