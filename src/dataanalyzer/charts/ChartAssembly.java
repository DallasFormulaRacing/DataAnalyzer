...
import dataanalyzer.data.LapDataDialog;
...
public class ChartAssembly implements ChartMouseListener {
    ...
    public ChartAssembly(ChartManager manager) {        
        ...
        //set necessary theme
        if(((DataAnalyzer)this.manager.getParentFrame()).getCurrTheme() == Theme.DARK) {
    ...
    */
    ...
    public void setChart(String[] tags) {
        ...
        //reset overlays
        overlay = new MyCrosshairOverlay(this);
        overlay.rangeMarkersActive = ((DataAnalyzer) manager.getParentFrame()).isRangeMarkersActive();
        this.xCrosshair = new Crosshair(Double.NaN, Color.GRAY, new BasicStroke(0f));
        this.xCrosshair.setLabelVisible(true);
        overlay.addDomainCrosshair(xCrosshair);
        yCrosshairs.clear();
    ...
    public void chartMouseClicked(ChartMouseEvent cme) {
        ...
        if(manager.getLapBreakerActive() >= 0) { 
            ...
            if(manager.getLapBreakerActive() == 0) {
                //round start time
                manager.getNewLap().setStart((long) xVal);
                //move to next task
            ...
            //if we are doing the second
                ...
                //get stop position
                manager.getNewLap().setStop((long) xVal);
                ...
                if(manager.getNewLap().getLapLabel().equals("!#@$LAPCANCELLED")) {
    ...
    */
    ...
    private long getRoundedTime(Dataset dataset, double val) {
        ...
        //find base function
            ...
            if(dataset.getDataMap().getTags().contains("Time," + goTo)) {
        ...
        */
        ...
        public void paintOverlay(Graphics2D g2, ChartPanel chartPanel) {
            ...
            for( Crosshair c : shadowD ) {
                ...
                if(((DataAnalyzer)this.parent.manager.getParentFrame()).getCurrTheme() == Theme.DARK)
